...
.PHONY: gen
gen:
	cd gen && acton build --dev $(DEP_OVERRIDES) && out/bin/orchino_gen
...
download-release:
	...
	mkdir -p out/bin; \
	if ! curl -L -f -o /tmp/$$RELEASE_FILE https://github.com/orchestron-orchestrator/orchino/releases/download/tip/$$RELEASE_FILE; then \
